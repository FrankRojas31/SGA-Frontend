...
export const LoginAccount = async(email: string, password: string) => {
  return await GenericRequest({url: `${urlBase}users/login`, method: "POST", data: {
    "email": email,
    "password": password
  }});
};
...
export const RegisterAccount = async(email: string, password:string) => {
  return await GenericRequest({url: `${urlBase}users/register`, method: "POST", data: {
    "email": email,
    "password": password
  }});
}
...
export const LogoutAccount = async(token: string) => {
  return await GenericRequest({url: `${urlBase}users/logout`, method: "POST", data: {
    "token": token
  }})
}
